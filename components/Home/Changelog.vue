<template>
  <div id="menu" class="flex w-[50rem] flex-col items-start justify-center gap-2 rounded-2xl bg-white p-10" @click.stop>
    ...
    <div class="h-96 w-full overflow-y-scroll p-4">
      <ol class="relative border-s border-gray-200 dark:border-gray-700">
        <li class="mb-10 ms-6" v-for="change in loadedChangelog" :key="change.version">
          <span class="absolute -start-3 flex size-7 items-center justify-center rounded-full bg-blue-100 ring-8 ring-white dark:bg-blue-900 dark:ring-gray-900">
            <img class="size-5" :src="change.type === 'bugfix' ? '/ui/bugFix.svg' : change.type === 'release' ? '/ui/majorRelease.svg' : '/ui/minorRelease.svg'" aria-hidden="true" />
          </span>
          <h3 class="mb-1 flex items-center text-lg font-semibold text-gray-900 dark:text-white">
            ...
            <span
              ...
              :class="{
                'dark:bg-orange-90 bg-orange-100 text-orange-800 dark:text-orange-300': change.type === 'release',
          ...
          <time class="mb-2 block text-sm font-normal leading-none text-gray-400 dark:text-gray-500"
            >Released on {{ new Intl.DateTimeFormat(undefined, { dateStyle: "long" }).format(new Date(change.release)) }}</time
          >
          <ul class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">
      ...
      >
        <img class="size-5" src="/ui/load.svg" aria-hidden="true" />
        Load more
      </button>
    </div>
    <p>
